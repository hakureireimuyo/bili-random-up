...
test("updateInterestFromWatch accumulates per tag", async () => {
  ...
  const updated = await updateInterestFromWatch(
    { tags: ["AI", "Tech"], watch_time: 50, duration: 100 },
    {
      updateInterestFn: async (tag, score) => {
        const profile = (await storage.getValue<Record<string, { tag: string; score: number }>>(
          "interestProfile"
        )) ?? {};
...
test("recommendUP returns highest score UP", async () => {
  ...
  const up = await recommendUP({
    getValueFn: (key: string) => storage.getValue(key)
  });
