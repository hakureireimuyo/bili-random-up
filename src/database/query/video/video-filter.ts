...
import type { VideoIndex, VideoQueryParams, Filter } from '../types.js';
...
export class VideoFilter {
  ...
  applyFilters(videos: VideoIndex[], params: VideoQueryParams): VideoIndex[] {
    ...
    // 创作者过滤
    if (params.filters) {
      const creatorFilter = params.filters.find((f: Filter) => f.field === 'creatorId');
      if (creatorFilter) {
    ...
    // 时间范围过滤
    if (params.filters) {
      const timeFilter = params.filters.find((f: Filter) => f.field === 'createdAt');
      if (timeFilter && timeFilter.operator === 'gte') {
        const endTimeFilter = params.filters.find((f: Filter) => 
