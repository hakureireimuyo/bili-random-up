import { __resetRateLimiter, apiRequest, getFollowedUPs, getUPInfo, getUPVideos, getVideoTags, rateLimiter } from "../api/bili-api.js";
...
function createFetchFail(status) {
    return async () => ({
        ok: false,
        status,
        json: async () => ({})
...
test("apiRequest returns data on success", async () => {
    ...
    const data = await apiRequest("/ok", { fetchFn });
